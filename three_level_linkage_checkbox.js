...
(function () {
    ...
    App.ThreeLevelLinkageCheckbox = {
        ...
        InitFirstCheckbox: function () {
             ...
             */
            var first_data_len = this.first_data.length;
            var firstCont = "";
        ...
        InitSecondCheckbox: function (i) {
             ...
             *  初始化二级标签
             */
            var second_data_i_len = this.second_data[i].length;
            var secondCont = "";
            for (var j = 0; j < second_data_i_len; j++) {
                if (this.second_data[i][j].status === "checked") {
                    secondCont += '<li onClick="App.ThreeLevelLinkageCheckbox.InitThirdCheckbox(' + i + ',' + j + ');">' +
                        ...
                        '</a>' +
                        '</li>';
                } else if (this.second_data[i][j].status === "half_checked") {
                    secondCont += '<li onClick="App.ThreeLevelLinkageCheckbox.InitThirdCheckbox(' + i + ',' + j + ');">' +
                        '<a href="javascript:void(0)">' +
                        ...
                        '<span>' + this.second_data[i][j].name + '</span>' +
                        '</a>' +
                        '</li>';
                } else {
                    secondCont += '<li onClick="App.ThreeLevelLinkageCheckbox.InitThirdCheckbox(' + i + ',' + j + ');">' +
           ...
           this.expressFirst = this.first_data[i];
            $("#selectedSort").html(this.expressFirst);
        ...
        InitThirdCheckbox: function (i, j) {
            ...
            var third_data_i_j_len = this.third_data[i][j].length;
            var thirdCont = "";
            for (var k = 0; k < third_data_i_j_len; k++) {
                if (this.third_data[i][j][k].status === "checked") {
                    thirdCont += '<li onClick="App.ThreeLevelLinkageCheckbox.InitForthCheckbox(' + i + ',' + j + ',' + k + ');">' +
                        ...
                        '<span>' + this.third_data[i][j][k].name + '</span>' +
                        '</a>' +
                        '</li>';
                } else {
                    thirdCont += '<li onClick="App.ThreeLevelLinkageCheckbox.InitForthCheckbox(' + i + ',' + j + ',' + k + ');">' +
            ...
            this.expressSecond = this.expressFirst + this.arrow + this.second_data[i][j];
            $("#selectedSort").html(this.expressSecond);
        ...
        FirstCheckboxClick: function (i) {
            ...
            this.first_data[i].status = $(checkbox_1_id).is(':checked') ? "checked" : "unchecked";

            //修改选择框样式
            if (this.first_data[i].status === "checked") {
            ...
            var second_data_i_len = this.second_data[i].length;//对应二级数据
            var third_data_i_len = this.third_data[i].length;//对应三级数据

            if (second_data_i_len > 0) {
                //二级有数据
                for (var j = 0; j < second_data_i_len; j++) {
                    this.second_data[i][j].status = this.first_data[i].status; //修改对应二级数据状态
                    if (third_data_i_len > 0) {
                        ...
                        if (third_data_i_j_len > 0) {
                            ...
                            for (var k = 0; k < third_data_i_j_len; k++) {
                                //修改对应三级数据状态
                                this.third_data[i][j][k].status = this.first_data[i].status;//修改三级数据状态
                            }
        ...
        SecondCheckboxClick: function (i, j) {
           ...
           this.second_data[i][j].status = $(checkbox_2_id).is(':checked') ? "checked" : "unchecked";
            ...
            if (third_data_i_j_len > 0) {
                ...
                for (var k = 0; k < third_data_i_j_len; k++) {
                    //修改对应三级数据状态
                    this.third_data[i][j][k].status = this.second_data[i][j].status;
                }
            }
            ...
            for (var m = 0; m < second_data_i_len; m++) {
                if (this.second_data[i][m].status === "checked") {
                    checked_sum++;
                } else if (this.second_data[i][m].status === "unchecked") {
                    unchecked_sum++;
                }
            }
            ...
            if (checked_sum === all_sum) {
                //全部选择
                this.first_data[i].status = "checked";
                $(checkbox_1_id).prop("checked", true);
                $(checkbox_1_id).next().css("background-color", "")
            } else if (unchecked_sum === all_sum) {
                //全部未选中
                this.first_data[i].status = "unchecked";
                $(checkbox_1_id).prop("checked", false);
                $(checkbox_1_id).next().css("background-color", "")
            } else {
                //部分选中
                this.first_data[i].status = "half_checked";
                $(checkbox_1_id).prop("checked", false);
        ...
        ThirdCheckboxClick: function (i, j, k) {
            ...
            this.third_data[i][j][k].status = $(checkbox_3_id).is(':checked') ? "checked" : "unchecked";
            ...
            var third_data_i_j_len = this.third_data[i][j].length;//二级下的全部三级数据
            ...
            if (third_data_i_j_len > 0) {
                ...
                for (var m = 0; m < third_data_i_j_len; m++) {
                    if (this.third_data[i][j][m].status === "checked") {
                        third_checked_sum++;
                    } else if (this.third_data[i][j][m].status === "unchecked") {
                        third_unchecked_sum++;
                    }
                }
            }
            ...
            if (third_checked_sum === third_all_sum) {
                //全部选中
                this.second_data[i][j].status = "checked";
                $(checkbox_2_id).prop("checked", true);
                $(checkbox_2_id).next().css("background-color", "")
            } else if (third_unchecked_sum === third_all_sum) {
                //全部未选中
                this.second_data[i][j].status = "unchecked";
                $(checkbox_2_id).prop("checked", false);
                $(checkbox_2_id).next().css("background-color", "")
            } else {
                //部分选中
                this.second_data[i][j].status = "half_checked";
                $(checkbox_2_id).prop("checked", false);
            ...
            if (second_data_i_len > 0) {
                ...
                for (var n = 0; n < second_data_i_len; n++) {
                    if (this.second_data[i][n].status === "checked") {
                        second_checked_sum++;
                    } else if (this.second_data[i][n].status === "unchecked") {
        ...
        GetChangeData: function () {
            return {
                first: this.first_data,
                second: this.second_data,
                third: this.third_data
            }
        }
    };
    //执行初始化函数
